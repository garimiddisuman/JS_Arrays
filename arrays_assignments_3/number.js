...
function getNumberToWords(placeValues) {
  const PLACE_WORDS = ['', ' thousand', ' laksh', ' crore'];
  const words = [];
...
function extractPlaceValues(number) {
  const placeValues = [];
  placeValues.unshift(number % 1000);
  let remainingDigits = Math.floor(number / 1000);

  while (remainingDigits > 0) {
    placeValues.unshift(remainingDigits % 100);
    remainingDigits = Math.floor(remainingDigits / 100);
  }
...
function aboveThousand() {
  ...
  testNumbetToWords(100001, 'one laksh one');
  testNumbetToWords(100000001, 'ten crore one');
  const segment = 'laksh thirty four thousand five hundred sixty seven';
  testNumbetToWords(18234567, 'one crore eighty two ' + segment);
  const segment1 = 'ninety nine thousand nine hundred ninety nine';
  testNumbetToWords(99999999, 'nine crore ninety nine laksh ' + segment1);
}
